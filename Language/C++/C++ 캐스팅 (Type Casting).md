...
- 다형성 환경에서 안전한 다운캐스팅
- 런타임 타입 체크 수행 (RTTI)
- 사용하려면 기반 클래스에 가상 함수가 반드시 존재해야 함
- 실패 시 nullptr 반환 (포인터 기준)
- 참조형 캐스팅 실패 시 std::bad_cast 예외 발생
...
- const로 선언된 변수를 수정하면 Undefined Behavior 발생
...
- 포인터를 다른 타입으로 재해석
- 메모리를 그대로 다른 타입으로 해석 (비트 단위)
- 컴파일 타임 타입 검사 없음
- 잘못된 타입으로 접근 시 Undefined Behavior 발생 가능
...
| 캐스팅           | 타입 체크     | 주요 용도                | 위험도 |
| ---------------- | ------------- | ------------------------ | ------ |
| static_cast      | 컴파일 타임   | 일반 타입 변환, 업캐스팅 | 낮음   |
| dynamic_cast     | 런타임 (RTTI) | 안전한 다운캐스팅        | 낮음   |
| const_cast       | 없음          | const/volatile 제거      | 중간   |
| reinterpret_cast | 없음          | 포인터 재해석            | 높음   |
