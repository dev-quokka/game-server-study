...
![OSI 7계층](../Operating%20System/images/osi-7-layers.webp)
...
```
7계층  →  클라이언트가 "공격" 버튼 누름 (HTTP or 커스텀 프로토콜)
6계층  →  해당 데이터를 직렬화 (JSON, Protobuf 등)
5계층  →  세션 유지 (이 플레이어 아직 접속 중)
4계층  →  TCP/UDP로 패킷 전송
3계층  →  서버 IP로 경로 탐색
2계층  →  MAC 주소로 다음 장치에 전달
1계층  →  0,1 신호로 전송
```
